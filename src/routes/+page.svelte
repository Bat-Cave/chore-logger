<script lang="ts">
	...
	const chores = {
		...
		// 	'Wipe TV'
		// ]
		outside: ['Picked Weeds', 'Watered Garden']
	};
	const householdMembers = ['Rico', 'Melissa', 'Natalie', 'Ben'];

	const startDay = new Date('2026-01-06');
	const daysToShow = 26;
		...
		<AspectRatio ratio={11 / 8.5} class="border">
			<div class="flex h-full w-full">
				<div class="grid grow grid-cols-11 grid-rows-9">
					...
					>
						<h1 class="text-2xl font-bold text-sky-700">Chore Logger</h1>
					...
					>
						...
						{#each days as day}
							<div
								class="relative flex w-full items-center justify-center border-r border-sky-500 last:border-r-0"
							>
								<div
								...
								>
									...
									<p class="text-sm font-semibold text-sky-700 uppercase">
										{format(day, 'ccccc')}
									</p>
					...
					>
						...
						{#each Object.entries(chores) as [room, roomchores], index}
							<div class="flex items-center justify-center bg-sky-500 text-white">
							...
							{#each roomchores as chore, choreIndex}
								<div
									...
									class="flex items-center border-b border-sky-500 px-2 last:border-b-0 data-[row-even='true']:bg-sky-100"
								>
					...
					>
						...
						{#each initialBoxes as box, index}
							...
							<div
								...
								style={`grid-template-columns: repeat(${days.length}, 1fr);`}
								class="flex flex-col items-center justify-center border-r border-b border-sky-500 px-2 text-xs data-[last-cell-in-column='true']:border-b-0 data-[last-cell-in-row='true']:border-r-0 data-[row-even='true']:bg-sky-100"
							></div>
						{/each}
					</div>
				</div>
